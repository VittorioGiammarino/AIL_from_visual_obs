...
class LailAgent:
    def __init__(self, state_shape, action_shape, device, gamma, batch_size_actor,
                ...
                num_expl_steps, reward_d_coef, lr_discriminator, stddev_schedule, stddev_clip, 
                GAN_loss='bce', from_dem=False):

        # Networks.
        self.actor = TruncatedGaussianPolicy(action_shape, num_sequences, feature_dim, hidden_units).to(device)
    ...
    def update(self, replay_iter, replay_iter_expert, step):
        ...
        if self.from_dem:
            ...
            with torch.no_grad():
                feature_actor = self.latent.encoder(state_).view(self.batch_size_actor, -1)
                feature_expert = self.latent.encoder(state_expert_).view(self.batch_size_actor, -1)
