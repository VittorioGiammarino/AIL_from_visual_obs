...
class Discriminator(nn.Module):
    def __init__(self, obs_dim, feature_dim, input_net_dim, hidden_dim, spectral_norm_bool=False, dist=None):
        super().__init__()
    ...
    def forward_dem(self, transition):
        obs_action = torch.tensor_split(transition, (self.obs_dim,), dim=-1)
        obs = obs_action[0]
        action = obs_action[1]
...
class Actor(nn.Module):
    def __init__(self, obs_dim, action_shape, feature_dim, hidden_dim):
        super().__init__()
...
class DrailPOMDPAgent:
    def __init__(self, obs_shape, action_shape, device, lr, feature_dim,
                 hidden_dim, critic_target_tau, num_expl_steps, discount,
                 update_every_steps, stddev_schedule, stddev_clip, use_tb, batch_size,
        ...
        self.critic = Critic(obs_shape, action_shape, feature_dim,
                             hidden_dim).to(device)
        ...
        if from_dem:
            if self.GAN_loss == 'least-square':
                self.discriminator = Discriminator(obs_shape, feature_dim, feature_dim+action_shape, 
            ...
            if self.GAN_loss == 'least-square':
                self.discriminator = Discriminator(2*obs_shape, 2*feature_dim, 2*feature_dim, hidden_dim, spectral_norm_bool).to(device)
                self.reward_d_coef = reward_d_coef

            elif self.GAN_loss == 'bce':
                self.discriminator = Discriminator(2*obs_shape, 2*feature_dim, 2*feature_dim, hidden_dim, spectral_norm_bool, dist='binary').to(device)
            else:
                NotImplementedError
    ...
    def act(self, obs, step, eval_mode):
        ...
        dist = self.actor(obs.float(), stddev)
        if eval_mode:
            action = dist.mean
        else:
    ...
    def update(self, replay_buffer, replay_buffer_expert, step):
        metrics = dict()

        if step % self.update_every_steps != 0:
            return metrics
        
        obs_a, action, reward_a, next_obs_a, not_done, not_done_no_max = replay_buffer.sample(self.batch_size)
        ...
        if self.from_dem:
            ...
            reward, metrics_r = self.compute_reward(obs_a, action)
        else:
            metrics.update(self.update_discriminator(obs_a, next_obs_a, obs_e, next_obs_e))
            reward, metrics_r = self.compute_reward(obs_a, next_obs_a)
        ...
        # update critic
        metrics.update(self.update_critic(obs_a, action, reward, next_obs_a, not_done_no_max, step))

        # update actor
        metrics.update(self.update_actor(obs_a.detach(), step))
