...
class ImagBehavior(nn.Module):
  ...
  def __init__(self, config, action_shape, world_model, actor_entropy, actor_state_entropy, imag_gradient_mix, stop_grad_actor=True, reward=None):
    ...
    if config.GAN_loss == 'bce':
      self.discriminator = DenseHead(feat_size + self.action_shape, [], config.discriminator_layers, config.units, act, dist='binary')
    elif config.GAN_loss == 'least-square':
      self.discriminator = DenseHead(feat_size + self.action_shape, [], config.discriminator_layers, config.units, act, dist='normal', std='learned')
  ...
  def _train(self, start, expert_data, objective=None, action=None, reward=None, imagine=None, tape=None, repeats=None):
    ...
    with utils.RequiresGrad(self.actor), utils.RequiresGrad(self.discriminator):
      with torch.cuda.amp.autocast(self._use_amp):
        ...
        post_expert, _ = self._world_model.dynamics.observe(embed_expert, expert_data['action'])
        feat_expert = self._world_model.dynamics.get_feat(post_expert)

        feat_expert_dist = torch.cat([feat_expert, expert_data['action']], -1)
        feat_policy_dist = torch.cat([imag_feat, imag_action], -1)
  ...
  def compute_discriminator_grad_penalty(self, policy_feat, expert_feat, lambda_=10):
      ...
      expert_feat_reshaped = torch.tile(torch.flatten(expert_feat, end_dim=-2).unsqueeze(0), [self._config.imag_horizon, 1, 1])
      disc_penalty_input = alpha*policy_feat + (1-alpha)*expert_feat_reshaped
...
class VmailAgent(nn.Module):
  ...
  def pretrain(self, steps):
    ...
    for _ in range(steps):
      ...
      mets_behavior = self.task_behavior._train(start, expert_data, reward)[-1]
  ...
  def update(self, step):
    ...
    reward = lambda f, s, a: self.world_model.heads['reward'](self.world_model.dynamics.get_feat(s)).mode()
    expert_data = next(self.expert_dataset)
    metrics.update(self.task_behavior._train(start, expert_data, reward)[-1])
